...
import {
  deleteActionOfProject,
  getDailyEffectiveActionsByProjectId, getVerifiedForBadge,
  postActionAtProject,
  postCopyRecords,
  putActionAtProject,
  rejectDailyReports,
...
function EffectiveActionsReportDailyPage({ date }: any) {
  ...
  const addFormik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      const res = await postActionAtProject(values);
      if (!(res instanceof Error)) {
        setAdd(null);
        toast.success("اقدام موثر با موفقیت درج شد");
        await refresh();
        // search(keySearch)
        resetForm();
      } else {
        toast.error("درج اقدام موثر با خطا مواجه شد");
      }
    },
  });
  const editFormik = useFormik({
    ...
    validationSchema: validation,
    onSubmit: async (values, { resetForm }) => {
      const res = await putActionAtProject({ ...values, Id: edit.id }, edit.id);
      if (!(res instanceof Error)) {
        setEdit(null);
        await refresh();
        toast.success("اقدام موثر با موفقیت ویرایش یافت");
        resetForm();
      } else {
        toast.error("ویرایش اقدام موثر با خطا مواجه شد");
      }
    },
  });
  ...
  const handleRemove = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const res = await deleteActionOfProject(remove, projectId);
    if (!(res instanceof Error)) {
      const tmpRows = [...rows];
      ...
      setRows(tmpRows);
      setRemove(null);
      toast.success("اقدام موثر با موفقیت حذف شد");
    } else {
      toast.error("حذف اقدام موثر با خطا مواجه شد");
    }
  };

  const handleVerify = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const res = await verifyCurrentAction(verify, projectId);
    if (!(res instanceof Error)) {
      const tmpRows = [...rows];
      ...
      setVerify(null);
      await refresh();
      toast.success("اقدام موثر با موفقیت تایید شد");
    } else {
      toast.error("تایید اقدام موثر با خطا مواجه شد");
    }
  };

  const handleVerifyAll = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const res = await verifyAllActions(projectId, date);
    if (!(res instanceof Error)) {
      const tmpRows = [...rows];
      ...
      setVerifyAll(false);
      await refresh();
      toast.success("تمام اقدامات موثر تایید شدند");
    } else {
      toast.error("تایید اقدامات موثر با خطا مواجه شد");
    }
  };

  const handleReject = async (event) => {
    event.preventDefault();
    const res = await rejectDailyReports(reject, rejectApiTypes.EffectiveAction, projectId);
    if (!(res instanceof Error)) {
  ...
  const refresh = async () => {
    const res = await getDailyEffectiveActionsByProjectId(projectId, date);
    if (!(res instanceof Error)) {
      setRows(res);
      setFilteredRow(res);
      // @ts-ignore
  ...
  const search = (key: string) => {
    if (key) {
      const filtered = rows.filter((row: any) => row.text.includes(key));
  ...
  return (
    <Grid container sx={{ position: "relative" }}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <TextField
                  ...
                  value={addFormik.values.Text}
                  error={addFormik.touched.Text && Boolean(addFormik.errors.Text)}
                  helperText={addFormik.touched.Text && addFormik.errors.Text}
                  fullWidth
                  multiline
                  maxRows={4}
                  minRows={2}
      ...
      {remove ? (
        <ModalIpa open={Boolean(remove)} title={`آیا از حذف اقدام موثر مطمئن هستید؟`} onClose={() => setRemove(null)}>
      ...
      {verify ? (
        <ModalIpa open={Boolean(verify)} title={`آیا از تایید اقدام موثر مطمئن هستید؟`} onClose={() => setVerify(null)}>
      ...
      {verifyAll ? (
        <ModalIpa
          ...
          title={`آیا از تایید تمام اقدامات موثر مطمئن هستید؟`}
...
export { EffectiveActionsReportDailyPage };
