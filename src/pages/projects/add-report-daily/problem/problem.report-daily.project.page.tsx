...
import {
  deleteProblemOfProject,
  getDailyProblemsByProjectId, getVerifiedForBadge,
  postCopyRecords,
  postProblemAtProject,
  putProblemAtProject,
  rejectDailyReports,
  verifyAllProblems,
...
function ProblemsReportDailyPage({ date }: any) {
  ...
  const addFormik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      const res = await postProblemAtProject(values);
      if (!(res instanceof Error)) {
        setAdd(null);
        toast.success("مشکل با موفقیت درج شد");
        await refresh();
        // search(keySearch)
        resetForm();
      } else {
        toast.error("درج مشکل با خطا مواجه شد");
      }
    },
  });
  const editFormik = useFormik({
    ...
    validationSchema: validation,
    onSubmit: async (values, { resetForm }) => {
      const res = await putProblemAtProject({ ...values, Id: edit.id }, edit.id);
      if (!(res instanceof Error)) {
        setEdit(null);
        await refresh();
        toast.success("مشکل با موفقیت ویرایش یافت");
        resetForm();
      } else {
        toast.error("ویرایش مشکل با خطا مواجه شد");
      }
    },
  });
  ...
  const handleRemove = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const res = await deleteProblemOfProject(remove, projectId);
    if (!(res instanceof Error)) {
      const tmpRows = [...rows];
      ...
      toast.error("حذف مشکل با خطا مواجه شد");
    }
  };

  const handleVerify = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const res = await verifyCurrentProblem(verify, projectId);
    if (!(res instanceof Error)) {
      const tmpRows = [...rows];
      ...
      setVerify(null);
      await refresh();
      toast.success("مشکل با موفقیت تایید شد");
    } else {
      toast.error("تایید مشکل با خطا مواجه شد");
    }
  };

  const handleVerifyAll = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const res = await verifyAllProblems(projectId, date);
    if (!(res instanceof Error)) {
      const tmpRows = [...rows];
      ...
      setVerifyAll(false);
      await refresh();
      toast.success("تمام مشکل ها تایید شدند");
    } else {
      toast.error("تایید مشکل ها با خطا مواجه شد");
    }
  };

  const handleReject = async (event) => {
    event.preventDefault();
    const res = await rejectDailyReports(reject, rejectApiTypes.Problem, projectId);
    if (!(res instanceof Error)) {
  ...
  const refresh = async () => {
    const res = await getDailyProblemsByProjectId(projectId, date);
    if (!(res instanceof Error)) {
      setRows(res);
      setFilteredRow(res);
      // @ts-ignore
  ...
  return (
    <Grid container sx={{ position: "relative" }}>
      ...
      {remove ? (
        <ModalIpa open={Boolean(remove)} title={`آیا از حذف مشکل مطمئن هستید؟`} onClose={() => setRemove(null)}>
      ...
      {verify ? (
        <ModalIpa open={Boolean(verify)} title={`آیا از تایید مشکل مطمئن هستید؟`} onClose={() => setVerify(null)}>
...
export { ProblemsReportDailyPage };
