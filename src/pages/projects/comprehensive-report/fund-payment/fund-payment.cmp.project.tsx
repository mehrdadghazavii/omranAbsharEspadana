...
import {getCostTypes, getCountersPartiesOfProject, getFundPaymentCMPReport, getPayersOfProject} from "api/api";
import 'date-fns';
...
function FundPaymentCMPPage({startDate, endDate}: any) {
  ...
  const handleFilterRows = () => {
    if (selectCntParties.length > 0 || selectPayers.length > 0 || selectCostTypes.length > 0)
      return rows.filter((row: any) => {
        if (selectCostTypes.includes(row.costTypeId) || selectPayers.includes(row.payerId) || selectCntParties.includes(row.counterPartyId))
          return true
        return false
      })
    return [...rows]
  ...
  const getAllNeedData = async () => {
    const res = await getFundPaymentCMPReport(projectId,
    ...
    if (!(res instanceof Error) && !(resCntParties instanceof Error) && !(resCostTypes instanceof Error) && !(resPayers instanceof Error)) {
      setRows(res)
      setFilteredRows(res)
      ...
      <Grid container spacing={2}>
        <Grid item md={6} xs={12}>
          <Autocomplete
              ...
              onChange={(e, v) => setSelectPayers(v.map((item: any) => item.id))}
              renderInput={(params) => (
                  <TextField
                      {...params}
                      variant="filled"
                           ...
                           if (head === 'pictureUrl') {
                             return (
                                ...
                                onMouseOut={() => {
                                   ...
                                   src={value} width={column.minWidth} alt={row.pictureName}
                                   style={{
                                    cursor: "pointer",
                                    width: "100%",
                                   }}
