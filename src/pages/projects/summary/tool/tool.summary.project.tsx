...
import {getToolSummaryReport} from "api/api";
...
function ToolSummaryPage({startDate, endDate}: any) {
  ...
  const getAllNeedData = async () => {
    const res: any = await getToolSummaryReport(projectId, startDate, endDate)
        ...
        <Grid item xs={12} style={{width: 10}}>
          <Paper>
            <TableProIpa columns={columns}
                         ...
                         summableColumns={["working", "notWorking"]}
                         rows={rows}/>
          </Paper>
        </Grid>
      </Grid>
